/*
 ...
 License: MIT
*/
(function(K,T,s){'use strict';function G(b){return function(){var a=arguments[0],c,a="["+(b?b+":":"")+a+"] http://errors.angularjs.org/1.3.0-beta.12/"+(b?b+"/":"")+a;for(c=1;c<arguments.length;c++)a=a+(1==c?"?":"&")+"p"+(c-1)+"="+encodeURIComponent("function"==typeof arguments[c]?arguments[c].toString().replace(/ \{[\s\S]*$/,""):"undefined"==typeof arguments[c]?"undefined":"string"!=typeof arguments[c]?JSON.stringify(arguments[c]):arguments[c]);return Error(a)}}function gb(b){if(null==b||Na(b))return!1;
...
return new O(b)}if(E(b)){var a;a=T;var c;b=(c=Ge.exec(b))?[a.createElement(c[1])]:(c=De(b,a))?c.childNodes:[]}pc(this,b)}function Jb(b){return b.cloneNode(!0)}function Ha(b){qc(b);for(var a=0,c=b.children,d=c&&c.length||0;a<d;a++)b=c[a],Ha(b)}function rc(b,a,c,d){if(F(d))throw Ib("offargs");var e=ja(b,"events");ja(b,"handle")&&(y(a)?q(e,function(a,c){Va(b,c,a);delete e[c]}):q(a.split(" "),function(a){y(c)?(Va(b,a,e[a]),delete e[a]):Ea(e[a]||[],c)}))}function qc(b,a){var c=b.ng,d=Wa[c];d&&(a?delete Wa[c].data[a]:
(d.handle&&(d.events.$destroy&&d.handle({},"$destroy"),rc(b)),delete Wa[c],b.ng=s))}function ja(b,a,c){var d=b.ng,d=Wa[d||-1];if(F(c))d||(b.ng=d=++He,d=Wa[d]={}),d[a]=c;else return d&&d[a]}function sc(b,a,c){var d=ja(b,"data"),e=F(c),f=!e&&F(a),g=f&&!P(a);d||g||ja(b,"data",d={});if(e){if(1===b.nodeType||9===b.nodeType)d[a]=c}else if(f){if(g)return d&&d[a];v(d,a)}else return d}function Kb(b,a){return b.getAttribute?-1<(" "+(b.getAttribute("class")||"")+" ").replace(/[\n\t]/g," ").indexOf(" "+a+" "):
!1}function lb(b,a){a&&b.setAttribute&&q(a.split(" "),function(a){b.setAttribute("class",$((" "+(b.getAttribute("class")||"")+" ").replace(/[\n\t]/g," ").replace(" "+$(a)+" "," ")))})}function mb(b,a){if(a&&b.setAttribute){var c=(" "+(b.getAttribute("class")||"")+" ").replace(/[\n\t]/g," ");q(a.split(" "),function(a){a=$(a);-1===c.indexOf(" "+a+" ")&&(c+=a+" ")});b.setAttribute("class",$(c))}}function pc(b,a){if(a)if(a.nodeType)b[b.length++]=a;else{var c=a.length;"number"===typeof c&&a.window!==a?
c&&tc.apply(b,a):b[b.length++]=a}}function uc(b,a){return nb(b,"$"+(a||"ngController")+"Controller")}function nb(b,a,c){b=z(b);9==b[0].nodeType&&(b=b.find("html"));for(a=L(a)?a:[a];b.length;){for(var d=b[0],e=0,f=a.length;e<f;e++)if((c=b.data(a[e]))!==s)return c;b=z(d.parentNode||11===d.nodeType&&d.host)}}function vc(b){for(var a=0,c=b.childNodes;a<c.length;a++)Ha(c[a]);for(;b.firstChild;)b.removeChild(b.firstChild)}function wc(b,a){var c=ob[a.toLowerCase()];return c&&xc[b.nodeName]&&c}function Ie(b,
...
function(b){return E(b)?b.trim():b}:function(b){return E(b)?b.replace(/^\s\s*/,"").replace(/\s\s*$/,""):b}}();Ka=9>U?function(b){b=b.nodeName?b:b[0];return b.scopeName&&"HTML"!=b.scopeName?Ga(b.scopeName+":"+b.nodeName):b.nodeName}:function(b){return b.nodeName?b.nodeName:b[0].nodeName};var ec=["ng-","data-ng-","ng:","x-ng-"],od=/[A-Z]/g,sd={full:"1.3.0-beta.12",major:1,minor:3,dot:0,codeName:"ephemeral-acceleration"};O.expando="ng";var Wa=O.cache={},He=1,sb=K.document.addEventListener?function(b,
...
s.label=d));G=null;C=0;for(D=k.length;C<D;C++)r=k[C],(d=w[C+1])?(G=d.element,d.label!==r.label&&G.text(d.label=r.label),d.id!==r.id&&G.val(d.id=r.id),d.selected!==r.selected&&G.prop("selected",d.selected=r.selected)):(""===r.id&&v?K=v:(K=E.clone()).val(r.id).attr("selected",r.selected).text(r.label),w.push({element:K,label:r.label,id:r.id,selected:r.selected}),G?G.after(K):s.element.append(K),G=K);for(C++;w.length>C;)w.pop().element.remove()}for(;z.length>y;)z.pop()[0].element.remove()}var k;if(!(k=
